...
def bucket(df, timeWindow=60):
    df['unixstarttime'] = pd.to_datetime(
        df.StartTime, format='%Y-%m-%dT%H:%M:%S').apply(unixTimetoSeconds)
    ...
    for index, raw in df.iterrows():
        ...
        if(x % 10 == 0):
            ...
            for i in range(diff):
                temp = [raw.StartTime, raw.Dur, raw.Proto, raw.SrcAddr, raw.Sport, raw.Dir, raw.DstAddr, raw.Dport, raw.State, raw.sTos*1.0/diff,
                        raw.dTos*1.0/diff, raw.TotPkts*1.0/diff, raw.TotBytes*1.0/diff, raw.SrcBytes*1.0/diff, raw.Label, raw.unixstarttime, start+i]
                ndf.loc[len(ndf)] = temp
    return ndf


df = pd.read_csv(
    "/home/ankit/Desktop/MTP/Database/CTU/capture20110810.binetflow")
ndf = bucket(df)
